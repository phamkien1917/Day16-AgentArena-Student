...
class CitationChecker(Middleware):
    ...
    def after_agent(self, ctx, report):
        if not isinstance(report, dict) or "claims" not in report or not isinstance(report["claims"], list) or ctx.corpus is None:
            return report
        ...
        for claim in claims:
            ...
            is_valid = False
            if doc is not None:
                is_valid = any(text in line for line in doc.body.splitlines())
            ...
            if is_valid or doc_id:
                new_citations.add(claim.get("doc_id", doc_id))
                
        report["citations"] = sorted(list(new_citations))
        return report
